...
from .dq.generation import QCQPDQCostFun
...
class LiDQBase(HERWCalibrationBase):
    ...
    def set_transforms(self, transforms_a: m3d.TransformContainer, transforms_b: m3d.TransformContainer,
        ...
        self.set_Mlist(dq.generation.gen_Mlist(transforms_a, transforms_b, cost_fun=QCQPDQCostFun.AX_YB))
...
class LiDQSignSampling(HERWCalibrationBase):
    ...
    def __init__(self, n_reps=1, n_samples=3):
        super().__init__()
        self._n_reps = n_reps
...
class LiDQSignBruteForce(HERWCalibrationBase):
    """| Simultaneous robot-world and hand-eye calibration using dual-quaternions and Kronecker product
    | A. Li, L. Wang and D. Wu
    | International Journal of the Physical Sciences (Vol. 5, Issue 10), 2010"""
    ...
    def _calibrate(self, **kwargs):
        if self._transforms_a is None:
            raise RuntimeError("Transformations are missing")
        self._method.configure(**kwargs)
        return dq.sign_ambiguity.calibrate_herw_sign_brute_force(self._method, self._transforms_a, self._transforms_b)


class LiDQSignInitHM(HERWCalibrationBase):
    """| Simultaneous robot-world and hand-eye calibration using dual-quaternions and Kronecker product
    | A. Li, L. Wang and D. Wu
    | International Journal of the Physical Sciences (Vol. 5, Issue 10), 2010"""
    ...
    @staticmethod
    def name():
        return 'LiDQSignInitHM'

    def __init__(self):
        super().__init__()
        self._init_method = LiHM()
        self._main_method = LiDQBase()
    ...
    def set_transforms(self, transforms_a: m3d.TransformContainer, transforms_b: m3d.TransformContainer,
        ...
        if weights is not None:
            raise RuntimeError("Weights are not supported by LiDQ")
        self._init_method.set_transforms(transforms_a, transforms_b)
        self._main_method.set_transforms(transforms_a, transforms_b)

    def _calibrate(self, **kwargs):
        self._main_method.configure(**kwargs)
        return dq.sign_ambiguity.calibrate_herw_sign_init(self._init_method, self._main_method)
