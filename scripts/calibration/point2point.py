...
def calibrate(method_cfg, points_a, points_b, calib_x):
    # initialize method
    method = method_cfg.create(excalibur.calibration.Point2PointCalibrationBase)
    method.set_points(points_a.T, points_b.T)
...
def main():
    # arguments
    parser = argparse.ArgumentParser(description="Point2Point Calibration")
    parser.add_argument('config', type=str, help="method configuration (*.yaml)")
    parser.add_argument('--a', type=str, nargs='+', required=True, help="files with points of sensor a (*.m3d)")
    parser.add_argument('--b', type=str, nargs='+', required=True, help="files with points of sensor b (*.m3d)")
    ...
    points_a, points_b = proc(transforms)
    print(f"Number of synchronous samples: {len(points_a)}")
    if len(points_a) < 3:
    ...
    # calibrate
    print("Calibrate")
    calib, metrics = calibrate(method, points_a, points_b, calib_gt)
    if calib is None:
        sys.exit(-1)
