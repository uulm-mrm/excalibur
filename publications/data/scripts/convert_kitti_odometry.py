...
def convert_kitti_odometry(kitti_root_dir, output_dir, binary):
    # iterate sequences
    for seq in SEQUENCES:
        ...
        # load data
        poses_data = np.loadtxt(osp.join(kitti_root_dir, 'poses', f'{seq_str}.txt'))
        times_data = np.loadtxt(osp.join(kitti_seq_dir, 'times.txt'))
        calib = load_kitti_odometry_calib(osp.join(kitti_seq_dir, 'calib.txt'), KittiFrame.CAM0)
        ...
        for row in range(poses_data.shape[0]):
            ...
            transform = m3d.MatrixTransform(poses_data[row, :], unsafe=True).normalized_()
            tdata.insert(timestamp, transform)

        # create motion data
        motion_data = m3d.MotionData(m3d.TransformType.kMatrix, tdata, calib, 'cam0')
...
def main():
    ...
    convert_kitti_odometry(args.kitti_dir, args.output_dir, args.binary)
